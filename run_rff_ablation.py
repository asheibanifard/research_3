...
def main():
    parser = argparse.ArgumentParser(description='Run all RFF ablation experiments defined in a YAML config')
    parser.add_argument('--config',   type=str, default='configs/rff_ablation_runs.yml')
    parser.add_argument('--no_wandb',   action='store_true')
    parser.add_argument('--use_kernel', action='store_true', help='Enable fused CUDA RFF kernel')
    ...
    for i, run in enumerate(runs):
        params   = {**defaults, **run}
        arch_tag = (f"n{params['hidden_layers']}{params['hidden_features']}"
                    f"_m{params['rff_features']}_s{int(params['rff_scale'])}")

        cmd = [
            sys.executable, '-m', 'src.rff',
            ...
            '--rff_features',            str(params['rff_features']),
            '--rff_scale',               str(params['rff_scale']),
