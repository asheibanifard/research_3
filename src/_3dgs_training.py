...
def train_impl(
    ...
    *,
    ...
    init_pts = dataset.swc_init_points().to(device) if getattr(cfg, 'swc_init', True) else None
    total_steps = cfg.epochs * cfg.steps_per_epoch
    gc = gaussian_cloud_cls(cfg.n_init, aabb, device, cfg, init_pts=init_pts)
    optimizer = make_optimizer(gc, cfg)
    update_lr(optimizer, 0, total_steps, cfg)
    run_date = datetime.now().strftime("%Y%m%d")
    init_path = out_dir / f"init_{run_date}.pth"
    best_path = out_dir / f"best_{run_date}.pth"
    final_path = out_dir / f"final_{run_date}.pth"
    gc.save(init_path)
    ...
    for epoch in epoch_bar:
        ...
        for _ in range(cfg.steps_per_epoch):
            ...
            loss, stats = compute_loss(pred, gt, gc, cfg, dataset)
            ...
            gc.clamp_means()
            gc.clamp_scales(cfg.scale_max_hard)
            ...
            if cfg.ckpt_interval > 0 and step % cfg.ckpt_interval == 0:
                gc.save(out_dir / f"ckpt_{run_date}_{step:07d}.pth")
