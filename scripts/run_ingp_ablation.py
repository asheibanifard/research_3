...
def main():
    parser = argparse.ArgumentParser(description='Run all Instant NGP ablation experiments')
    parser.add_argument('--config',   type=str, default='configs/ingp_ablation_runs.yml')
    parser.add_argument('--no_wandb', action='store_true')
    parser.add_argument('--use_kernel', action='store_true', help='Enable fused CUDA hash-encoding kernel')
    ...
    for i, run in enumerate(runs):
        params   = {**defaults, **run}
        arch_tag = (f"h{params['n_levels']}f{params['n_features_per_level']}"
                    f"T{params['log2_hashmap_size']}_l{params['hidden_layers']}n{params['hidden_features']}")

        cmd = [
            sys.executable, '-m', 'src.ingp',
            '--input',                   str(params['input']),
            '--model_dir',               str(params['model_dir']),
            '--n_levels',                str(params['n_levels']),
            '--n_features_per_level',    str(params['n_features_per_level']),
            '--log2_hashmap_size',       str(params['log2_hashmap_size']),
            '--base_resolution',         str(params['base_resolution']),
            '--finest_resolution',       str(params['finest_resolution']),
